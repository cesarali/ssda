...
from ssda.models.classifier_config import ClassifierConfig
from ssda.configs.files_config import ExperimentFiles

from ssda.trainers.ssvae_trainer_config import SSVAETrainerConfig
from ssda.data.dataloaders_config import NISTLoaderConfig,SemisupervisedLoaderConfig

all_dataloaders_configs = {"NISTLoader":NISTLoaderConfig,
                           "SemisupervisedLoader":SemisupervisedLoaderConfig}
...
class SSVAEConfig:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.classifier,dict):
            self.classifier = all_classifiers_configs[self.classifier["name"]](**self.classifier)
...
def get_config_from_file(experiment_name, experiment_type, experiment_indentifier) -> SSVAEConfig:
    ...
    if results_dir_path.exists():
        ...
        config = SSVAEConfig(**config_path_json)
        return config
    else:
